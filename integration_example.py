...
async def handle_set_vin_button(
        update: Update,
...
async def save_group_vin_implementation(
        group_id: int,
        vin: str,
    ...
    try:
        if hasattr(
                context.bot_data,
                'google_integration') and context.bot_data.google_integration:
            ...
            try:
                ...
                if row_to_update:
                    ...
                    google.groups_worksheet.update(
                        f'C{row_to_update}', [[vin]])
                    logger.info(
                        f"Updated existing group {group_id} with VIN {vin}")
                else:
                    ...
                    logger.info(
                        f"Created new group record {group_id} with VIN {vin}")
