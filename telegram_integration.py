...
async def _global_error_handler(update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Best-effort: if it's a callback query, try to answer so Telegram
        # doesn't keep spinning
...
class SessionData:
    ...
    # (lat, lng) for location-based stop detection
...
class EnhancedLocationBot(RiskDetectionMixin):
    ...
    def __init__(self, config: Config):
        ...
        self.session_timeout_hours = getattr(
        ...
        def decorator(func):
            ...
            async def wrapper(
                ...
                try:
                    ...
                    error_message = str(e)
                    logging.getLogger(__name__).error(
                        f"Error in command {command_name}: {e}")
                    raise
                    ...
                    try:
                        ...
                        logging.getLogger(__name__).warning(
                            f"Failed to log command {command_name}: {log_error}")
    ...
    def __post_init__(self):
        ...
        # VIN -> acknowledgment time
        self.acknowledged_alerts: Dict[str, datetime] = {}
        self.acknowledgment_duration = timedelta(
            hours=24)  # Fixed 24-hour acknowledgment duration

        # ETA alert muting system
        self.mute_store: Dict[str, datetime] = {}  # Alert key -> expiry time

        # Different intervals for different purposes
        # 1 hour for location updates
        self.group_location_interval = self.config.GROUP_LOCATION_INTERVAL
        # 5 minutes for ETA tracking
        self.live_tracking_interval = self.config.LIVE_TRACKING_INTERVAL
        ...
        if RISK_DETECTION_AVAILABLE:
            try:
                ...
                logger.info(
                    "Risk detection initialized successfully in __post_init__")
            except Exception as e:
                logger.error(
                    f"Failed to initialize risk detection in __post_init__: {e}")
    ...
    async def restore_group_schedules(
        ...
        if ETA_SERVICE_AVAILABLE:
            ...
            logger.warning(
                "ETA service not available - continuing without ETA alerting")
            self.eta_service = None
    ...
    def cleanup_acknowledged_alerts(self):
        ...
        if expired_vins:
            logger.info(
                f"Cleaned up {len(expired_vins)} expired acknowledgments")

        # Also cleanup muted alerts
        self._cleanup_muted_alerts()

    async def handle_risk_alert_callback(
            self,
            update: Update,
            context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            if callback_data.startswith("ack_alert_"):
                ...
                logger.info(
                    f"Risk alert acknowledged for VIN {vin} by user {query.from_user.id}")
    ...
    async def _handle_contact_driver(self, query, vin: str):
        ...
        try:
            ...
            if hasattr(self.google_integration, 'get_driver_contact_info'):
                try:
                    name, phone = self.google_integration.get_driver_contact_info(
                        vin)
                    if name:
                        driver_name = name
                    if phone:
    ...
    def _cleanup_muted_alerts(self):
        ...
        expired_keys = [
            key for key,
            expiry in self.mute_store.items() if current_time > expiry]
        ...
        if expired_keys:
            logger.debug(
                f"Cleaned up {len(expired_keys)} expired muted alerts")

    async def _handle_eta_late_ack(
            self,
            query,
            callback_data: str,
            alert_type: str):
        ...
        try:
            ...
            logger.info(
                f"{alert_type_display} late alert acknowledged: {callback_data}")
    ...
    def _schedule_group_location_updates(
        ...
        logger.info(
            f"Individual group scheduling disabled for chat {chat_id} - using centralized GroupUpdateScheduler")

        # Mark session as enabled for compatibility but don't create individual
        # jobs
        session = self.get_session(chat_id)
        session.auto_refresh_enabled = True
    ...
    def _schedule_live_eta_tracking(
            self,
            chat_id: int,
            context: ContextTypes.DEFAULT_TYPE):
        ...
        if not context.job_queue:
            logger.error(
                f"Job queue not available for chat {chat_id}, cannot schedule ETA tracking")
            return
        ...
        try:
            ...
            logger.info(
                f"Scheduled SILENT data refresh for chat {chat_id} every {self.live_tracking_interval}s (no messages)")
        except Exception as e:
            logger.error(
                f"Failed to schedule silent data refresh for chat {chat_id}: {e}")

    async def _group_location_callback(
            self, context: ContextTypes.DEFAULT_TYPE):
        """DEPRECATED: Individual group callback - now handled by centralized GroupUpdateScheduler"""
        logger.warning(
            "Old individual group callback triggered - this should not happen with centralized scheduler")
    ...
    async def _live_eta_callback(self, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if not all([session.vin, session.stop_address]):
                logger.debug(
                    f"Skipping silent ETA refresh for chat {chat_id} - incomplete data")
                return
            ...
            if not truck:
                logger.warning(
                    f"Truck {session.vin} not found during silent refresh for chat {chat_id}")
                return
            ...
            if old_lat != session.lat or old_lng != session.lng:
                logger.debug(
                    f"Silent location update for chat {chat_id}: {old_lat},{old_lng} → {session.lat},{session.lng}")
    ...
    def _update_stop_duration_tracking(
            self,
            session: SessionData,
            truck: dict,
            current_time_edt: datetime) -> Optional[str]:
        ...
        # Consider stopped if speed is less than 2 mph and location hasn't
        # changed significantly
        is_currently_stopped = current_speed < 2.0

        # Check if location has changed significantly (more than ~50 meters)
        if session.last_location and current_location and current_location[
                0] and current_location[1]:
            lat_diff = abs(current_location[0] - session.last_location[0])
            lng_diff = abs(current_location[1] - session.last_location[1])
            location_changed = (
                lat_diff > 0.0005) or (
                lng_diff > 0.0005)  # ~50m threshold

            # If location changed significantly, consider it moving regardless
            # of speed
        ...
        if is_currently_stopped:
            if not session.is_stopped:
                ...
                logger.info(
                    f"Driver {session.vin} stopped at {current_time_edt}")
            else:
            ...
            if session.is_stopped:
                ...
                if session.stop_start_time and not session.last_stop_duration_shown:
                    ...
                    logger.info(
                        f"Driver {session.vin} resumed after {duration_str} stop")
    ...
    def _get_stop_status_indicator(
            self,
            session: SessionData,
    ...
    async def _send_group_location_update(
            self,
            context: ContextTypes.DEFAULT_TYPE,
            chat_id: int,
            session: SessionData,
        ...
        try:
            ...
            driver_name = self.google_integration.get_driver_name_by_vin(
                session.vin) or session.driver_name or 'Unknown Driver'
            ...
            if truck.get('update_time'):
                try:
                    ...
                    updated_at_utc = datetime.now(
                        ZoneInfo('UTC'))  # Simplified for now
                except Exception:
            ...
            if session.stop_address:
                try:
                    ...
                    dest_coords = self.tms_integration.geocode(
                        session.stop_address)
                    if dest_coords:
                        origin = [session.lng, session.lat]
                        route = self.tms_integration.get_route(
                            origin, dest_coords)
                        ...
                        if route:
                            ...
                            if session.appointment:
                                try:
                                    ...
                                    appt_str = session.appointment.replace(
                                        "EDT", "").replace("EST", "").strip()
                                    from datetime import datetime as dt
                                    appt_time_naive = dt.strptime(
                                        appt_str, "%I:%M %p")
            ...
            keyboard = [
                [
                    InlineKeyboardButton(
                        "📍 Set Stop Location",
                        callback_data=CB_SEND_STOP),
                    InlineKeyboardButton(
                        "⏰ Set Appointment",
                        callback_data=CB_SEND_APPOINTMENT)]]

            # Add Calculate ETA button if we have stop location
            if session.stop_address:
                keyboard.insert(0, [InlineKeyboardButton(
                    "↪️ Calculate ETA", callback_data=CB_CALCULATE_ETA)])
            else:
                keyboard.insert(0, [InlineKeyboardButton(
                    "💡 Set Stop Location for ETA", callback_data=CB_SEND_STOP)])
    ...
    def _get_data_age_warning(self, truck: dict) -> str:
        ...
        try:
            ...
            logger.debug(
                f"Could not parse TMS update time '{update_time_str}': {e}")
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if chat_type != 'private':
            ...
            if vin:
                ...
                # Start automatic location updates for registered groups (with
                # error handling)
                try:
                    if context.job_queue:
                        ...
                        if RISK_DETECTION_AVAILABLE and hasattr(
                                self, 'enable_risk_monitoring') and self.enable_risk_monitoring:
                        ...
                        logger.warning(
                            f"Job queue not available for group {chat_id}, auto-updates disabled")
                        welcome_msg = (
                            "🚚 **Asset Tracking Bot - Group Ready**\n\n"
                            f"✅ VIN registered: {vin}\n"
                    ...
                    logger.error(
                        f"Error scheduling group updates for {chat_id}: {e}")
                    welcome_msg = (
                        "🚚 **Asset Tracking Bot - Group Ready**\n\n"
                        f"✅ VIN registered: {vin}\n"
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Failed to send start message to {chat_id}: {e}, fallback: {fallback_e}")

    def _build_main_menu(self,
                         chat_type: str,
                         chat_id: int,
                         session: SessionData,
                         user_id: Optional[int] = None) -> List[List[InlineKeyboardButton]]:
        ...
        if chat_type == 'private':
            ...
            keyboard = [
                [
                    InlineKeyboardButton(
                        "🛰 Track Driver", callback_data=CB_GET_UPDATE)], [
                    InlineKeyboardButton(
                        "📍 Set Stop Location", callback_data=CB_SEND_STOP), InlineKeyboardButton(
                        "⏰ Set Appointment", callback_data=CB_SEND_APPOINTMENT)], [
                            InlineKeyboardButton(
                                "↪️ Calculate ETA", callback_data=CB_CALCULATE_ETA)], ]
        else:
            # Group chat menu
        ...
        # Common buttons
        keyboard.extend([[InlineKeyboardButton("🆘 Help",
                                               callback_data=CB_HELP),
                          InlineKeyboardButton("📬 Contact Admin",
                         callback_data=CB_ADMIN_CONTACT)]])
    ...
    def _save_group_vin(
        ...
        try:
            success = self.google_integration.save_group_vin(
                group_id, group_title, vin, driver_name)
            if success:
                logger.info(
                    f"Successfully saved VIN for group {group_id}: {vin}")
            else:
    ...
    async def button_router(
        ...
        # Check if callback is too old (Telegram callbacks expire after ~1
        # hour)
        if query and hasattr(
                query,
                'message') and query.message and hasattr(
                query.message,
                'date'):
            callback_age = (
                datetime.now(
                    query.message.date.tzinfo) -
                query.message.date).total_seconds()
            # 55 minutes (give 5 min buffer before 1hr expiry)
            if callback_age > 3300:
                logger.warning(
                    f"CallbackQuery too old ({callback_age:.0f}s), skipping")
                try:
        ...
        try:
            ...
            # NEW RISK-RELATED CALLBACKS
                ...
                logger.info(
                    f"Driver selection button clicked: {callback_data}")
    ...
    async def _send_manual_location_update(
            self,
            update: Update,
            context: ContextTypes.DEFAULT_TYPE,
            vin: str):
        ...
        try:
            ...
            if not truck:
                # Check why the truck wasn't found (may be filtered due to old
                # data)
                try:
    ...
    async def _handle_set_vin(
        ...
        try:
            ...
            # First attempt auto-registration (won't send message if no
            # high-confidence match)
            await auto_register_vin_on_group_join(update, context)

            # Check if auto-registration succeeded by checking if VIN is now
            # set
            ...
            # Fallback to old manual method if VIN suggestion system isn't
            # available
            logger.warning(
                "VIN suggestion system not available, using manual entry")
            chat_id = update.effective_chat.id
            session = self.get_session(chat_id)
    ...
    async def _handle_send_stop_location(
            self,
            update: Update,
            context: ContextTypes.DEFAULT_TYPE):
    ...
    async def _handle_send_appointment(
            self,
            update: Update,
            context: ContextTypes.DEFAULT_TYPE):
    ...
    async def _handle_calculate_eta(
            self,
            update: Update,
            context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if not truck:
                ...
                # data)
                try:
                    vin_status = self.tms_integration.check_vin_status(
                        session.vin)
            ...
            if context.job_queue:
                ...
                logger.info(
                    f"Switched chat {chat_id} to live ETA tracking mode")
    ...
    async def _handle_admin_contact(
            self,
            update: Update,
            context: ContextTypes.DEFAULT_TYPE):
    ...
    async def _handle_stop_auto_refresh(
            self,
            update: Update,
            context: ContextTypes.DEFAULT_TYPE):
    ...
    async def _handle_start_auto_refresh(
            self,
            update: Update,
            context: ContextTypes.DEFAULT_TYPE):
    ...
    async def _handle_status(
        ...
        active_sessions = len(
            [s for s in self.sessions.values() if s.auto_refresh_enabled])
        ...
        try:
            ...
            sample_text = "\n".join(
                [f"• {name}" for name in sample_drivers]) if sample_drivers else "No drivers found"
        except Exception as e:
        ...
        if RISK_DETECTION_AVAILABLE and hasattr(self, 'risk_detector'):
            risk_zones = len(
                self.risk_detector.risk_zones) if hasattr(
                self.risk_detector,
                'risk_zones') else 0
    ...
    async def _handle_risk_status(
        ...
        try:
            ...
            ack_list = []
            for vin, ack_time in self.acknowledged_alerts.items():
                time_left = self.acknowledgment_duration - \
                    (datetime.now() - ack_time)
            ...
            # QC Panel status
            qc_panel_status = "✅ Configured" if getattr(
                self.config, 'QC_PANEL_SPREADSHEET_ID', None) else "❌ Not configured"
    ...
    async def update_all_groups(
        ...
        try:
            ...
            groups_records = self.google_integration._get_groups_records_safe()
            logger.info(
                f"Retrieved {len(groups_records)} total records from groups sheet")

            # Debug: Show first few records
            if groups_records:
                logger.debug(
                    f"Sample record: {groups_records[0] if groups_records else 'No records'}")
            ...
            for i, record in enumerate(groups_records):
                ...
                if group_id and vin and is_active:
                    try:
                        ...
                        logger.debug(
                            f"Skipped record {i} due to conversion error: {e}")
                        skipped_count += 1
                        continue
                else:
                    skipped_count += 1
            ...
            for i, group in enumerate(groups):
                ...
                if not group_id or not vin:
                    error_count += 1
                    errors.append(
                        f"Group {group_id or 'Unknown'}: Missing VIN or ID")
                    continue
                ...
                try:
                    ...
                    if not truck:
                        error_count += 1
                        errors.append(
                            f"Group {group_id}: Truck not found for VIN {vin}")
                        continue
    ...
    async def update_assets_command(
            self,
            update: Update,
            context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            # Call the Google integration method to update assets (no limit -
            # process all trucks)
    ...
    async def list_new_trucks_command(
            self,
            update: Update,
            context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if len(success_msg) > 4000:
                ...
                # Send VINs list
                vins_list = "\n".join(
                    [f"`{truck['vin']}`" for truck in new_trucks[:10]])
    ...
    async def groups_diagnostic_command(
            self,
            update: Update,
            context: ContextTypes.DEFAULT_TYPE):
                ...
                try:
                    ...
                    try:
                        ...
                        if records:
                            ...
                            for key in list(
                                    sample.keys())[:8]:  # Show first 8 keys
    ...
    async def validate_data_command(
            self,
            update: Update,
            context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            try:
                    ...
                    if vin:
                        ...
                        if vin in driver_vin_map and driver_vin_map[vin] != driver:
                            validation_results.append(
                                f"❌ **VIN {vin}** mapped to multiple drivers: '{driver_vin_map[vin]}' and '{driver}'")
                        else:
                            driver_vin_map[vin] = driver

                # Find duplicate VINs
                duplicate_vins = [vin for vin,
                                  count in vin_counts.items() if count > 1]
                for vin in duplicate_vins:
                    validation_results.append(
                        f"⚠️ **Duplicate VIN:** {vin} appears {vin_counts[vin]} times")
                ...
                if test_vin in driver_vin_map:
                    ...
                    validation_results.append(
                        f"ℹ️ **VIN {test_vin}** is mapped to: '{actual_driver}'")
    ...
    async def worksheets_health_command(
            self,
            update: Update,
            context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            monitor = create_worksheet_monitor(
                self.google_integration, self.config)
    ...
    async def auto_register_groups_command(
    ...
    async def handle_text_message(
        ...
        if session.current_state == ASK_DRIVER_NAME:
            ...
            # No active conversation state - only respond in groups if
            ...
            if chat_type == 'private':
                ...
                # In groups, only respond if bot was mentioned or message is a
                # reply to bot
    ...
    async def _process_driver_name(
            self,
            update: Update,
            context: ContextTypes.DEFAULT_TYPE,
            driver_name: str):
        ...
        try:
            ...
            logger.info(
                f"Looking up driver '{driver_name}' in Google Sheets...")
            ...
            should_show_fuzzy = (
                ...
                # Short single word
                (len(driver_name.strip()) <= 8 and " " not in driver_name.strip())
            )

            if should_show_fuzzy:
                logger.info(
                    f"Showing fuzzy matching for input '{driver_name}' (vin_found: {bool(vin)}, length: {len(driver_name)})")
                # Try to provide helpful suggestions from Google Sheets
                suggestions = self._find_similar_driver_names_from_sheets(
                    driver_name)

                if suggestions:
                    logger.info(
                        f"Creating fuzzy matching buttons for {len(suggestions)} driver suggestions")
                    # Create inline buttons for suggestions
                    keyboard = []
                    for name in suggestions[:5]:  # Limit to 5 suggestions
                        # Truncate long names for button display, but be
                        # smarter about it
                        if len(name) > 25:
                            # Try to keep important parts (names before
                            # slashes)
                            parts = name.split(' / ')
                            if len(parts) > 1:
                                display_name = parts[0][:15] + \
                                    "..." if len(parts[0]) > 15 else parts[0]
                                display_name += f" / {parts[-1][:8]}..." if len(
                                    parts[-1]) > 8 else f" / {parts[-1]}"
                            else:
                        ...
                        logger.debug(
                            f"Creating button: '{display_name}' with callback: '{callback_data}'")
                        keyboard.append([InlineKeyboardButton(
                            f"👤 {display_name}", callback_data=callback_data)])
                    ...
                    logger.info(
                        f"Sending fuzzy matching message with {len(keyboard)} buttons")
            ...
            logger.info(
                f"Found VIN {vin} for driver {driver_name}, verifying mapping...")

            # Check if this VIN actually belongs to a different driver in the
            # sheets
            try:
            ...
            logger.info(
                f"Successfully found driver {driver_name} at {session.lat}, {session.lng}")

            # Send location update - handle both message and callback query
            # contexts
    ...
    def _find_similar_driver_names_from_sheets(
            self, search_name: str) -> List[str]:
        """Find similar driver names from Google Sheets for suggestions"""
        try:
            return self.google_integration.find_similar_driver_names(
                search_name)
        except Exception as e:
            logger.error(f"Error finding similar names for {search_name}: {e}")
            return []

    async def _handle_driver_selection(
            self,
            update: Update,
            context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    f"Could not edit fuzzy matching message: {edit_error}")
    ...
    async def _send_private_location_update(
            self,
            update: Update,
            context: ContextTypes.DEFAULT_TYPE,
            session: SessionData,
        ...
        try:
            map_url = f"https://maps.google.com/?q={session.lat},{session.lng}"
            speed_display = self._format_speed_for_display(
                truck.get('speed', 0))
            current_speed = self._normalize_speed(truck.get('speed', 0))
            ...
            if tms_driver and tms_driver != 'Not in TMS' and tms_driver.lower() != sheets_driver.lower():
                logger.warning(
                    f"Driver name mismatch for VIN {session.vin}: TMS='{tms_driver}' vs Sheets='{sheets_driver}'")
            ...
            edt_tz = pytz.timezone('America/New_York')
            updated_time_edt = session.last_updated.replace(tzinfo=pytz.utc).astimezone(
                edt_tz) if session.last_updated else datetime.now(edt_tz)
            ...
            keyboard = [
                [
                    ...
                    InlineKeyboardButton(
                        "↪️ Calculate ETA", callback_data=CB_CALCULATE_ETA)], [
                            InlineKeyboardButton(
                                "🏠 Main Menu", callback_data=CB_BACK_TO_MAIN)]]

            logger.debug(
                f"Sending private location update with {len(keyboard)} button rows")
            ...
            if update.message:
                ...
                logger.error(
                    "Unable to determine send method - no message or callback_query")
                return
            ...
            try:
                ...
                logger.info(
                    "Private location update sent successfully with inline buttons")

                # Clean up any progress message from fuzzy matching selection
                try:
                    progress_message = context.user_data.get(
                        'progress_message') if context.user_data else None
                    if progress_message:
                        ...
                        logger.debug(
                            "Cleaned up progress message after location update")
                except Exception as cleanup_error:
                    logger.debug(
                        f"Could not clean up progress message: {cleanup_error}")

            except Exception as button_error:
                logger.error(
                    f"Failed to send buttons with location message: {button_error}")
                ...
                try:
                    ...
                    logger.error(
                        f"Fallback send also failed: {fallback_error}")
            ...
            try:
                # Get correct driver name from Google Sheets assets data
                fallback_driver_name = self.google_integration.get_driver_name_by_vin(
                    session.vin) or session.driver_name or 'Unknown'
    ...
    async def _process_vin(
        ...
        try:
            ...
            success = self._save_group_vin(
                chat_id, group_title, vin, driver_name)
    ...
    async def _process_stop_location(
            self,
            update: Update,
            context: ContextTypes.DEFAULT_TYPE,
            location: str):
...
def build_application(config: Config) -> Application:
    ...
    try:
        ...
        # Build the application with job queue enabled (correct method for
        # v20.8)
        application = (
            ApplicationBuilder()
        ...
        # Verify job queue is available (it should be enabled by default in
        # v20.8)
        if not application.job_queue:
            logger.warning(
                "Job queue not available, trying to create one manually")
            # Try to initialize job queue manually if needed
            try:
                from telegram.ext import JobQueue
                if not hasattr(application,
                               '_job_queue') or application._job_queue is None:
                    # Job queue should be created automatically, but let's
                    # ensure it exists
                    logger.info(
                        "Job queue will be created during application initialization")
        ...
        application.add_handler(
            CommandHandler(
                "updateall",
                enhanced_bot.update_all_groups))
        application.add_handler(
            CommandHandler(
                "updateassets",
                enhanced_bot.update_assets_command))
        application.add_handler(
            CommandHandler(
                "listnewtrucks",
                enhanced_bot.list_new_trucks_command))
        application.add_handler(
            CommandHandler(
                "addtruck",
                enhanced_bot.add_truck_command))
        application.add_handler(
            CommandHandler(
                "workshealth",
                enhanced_bot.worksheets_health_command))
        application.add_handler(
            CommandHandler(
                "autoregister",
                enhanced_bot.auto_register_groups_command))
        application.add_handler(
            CommandHandler(
                "groupsdiag",
                enhanced_bot.groups_diagnostic_command))
        application.add_handler(
            CommandHandler(
                "validatedata",
                enhanced_bot.validate_data_command))
        ...
        try:
            ...
            async def on_bot_added_to_group(
                ...
                if member_update and member_update.new_chat_member:
                    if member_update.new_chat_member.status in [
            ...
            logger.info(
                "VIN suggestion system with auto-registration integrated successfully")
...
# Add risk alert callback handlers (NEW) - only if risk detection is available
        if RISK_DETECTION_AVAILABLE:
            ...
            logger.info(
                "Risk alert callback handlers registered with ETA late acknowledgments")

        # Add text message handler for conversation states
        # This handler now includes the logic to ignore messages when
        # appropriate
        ...
        if RISK_DETECTION_AVAILABLE and hasattr(
                enhanced_bot, 'enable_risk_monitoring'):
            if enhanced_bot.enable_risk_monitoring:
                # Schedule risk monitoring to start after application is
                # running
        ...
        if enhanced_bot.session_timeout_hours and enhanced_bot.session_timeout_hours > 0:
            logger.info(
                f"Scheduling session cleanup every {enhanced_bot.session_timeout_hours} hours")

            # Schedule cleanup to run every hour (more frequent than timeout
            # for better cleanup)
            cleanup_interval_hours = min(
                1, enhanced_bot.session_timeout_hours / 2)
            ...
            if application.job_queue:
                ...
                logger.info(
                    f"Session cleanup scheduled to run every {cleanup_interval_hours} hours")
            else:
                logger.warning(
                    "Job queue not available, session cleanup will not be scheduled")
        ...
        if application.job_queue:
            ...
            # Schedule this to run once, shortly after startup (30 seconds
            # delay)
            ...
            logger.info(
                "Group schedule restoration scheduled to run 30 seconds after startup")
        else:
            logger.warning(
                "Job queue not available, group schedules will not be restored automatically")

        logger.info("Enhanced Telegram application built successfully")
        logger.info(
            f"Job queue available: {application.job_queue is not None}")
        ...
        if RISK_DETECTION_AVAILABLE and hasattr(enhanced_bot, 'risk_detector'):
            risk_zones = len(
                enhanced_bot.risk_detector.risk_zones) if hasattr(
                enhanced_bot.risk_detector,
                'risk_zones') else 0
            logger.info(f"Risk zones loaded: {risk_zones}")
            logger.info(
                f"Risk monitoring enabled: {getattr(enhanced_bot, 'enable_risk_monitoring', False)}")
            logger.info(
                f"Acknowledgment system: {len(enhanced_bot.acknowledged_alerts)} active acknowledgments")

        return application

    except Exception as e:
        logger.error(
            f"Failed to build Telegram application: {e}",
            exc_info=True)
        raise
