...
async def manual_eld_refresh():
    ...
    try:
        ...
        if updated_count > 0:
            print(f"✅ Successfully updated {updated_count} records in ELD_tracker")
            print(f"🕐 Timestamps should now show current time: {datetime.now(ZoneInfo('America/New_York')).strftime('%Y-%m-%d %H:%M:%S %Z')}")
            return True
        else:
