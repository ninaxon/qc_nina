...
def extract_names_from_title(title: str) -> List[str]:
    ...
    for part in parts:
        ...
        if (len(part) <= 3 or 
            ...
            re.match(r"^[A-Z]\*?$", part) or  # Single letter codes like "S*", "B*"
...
def build_assets_index(rows: List[List[str]], driver_col: int, vin_col: int) -> List[Tuple[str, str]]:
    """
    ...
    for row_idx, row in enumerate(data_rows):
        try:
            ...
            driver_name = str(row[driver_col]).strip() if row[driver_col] else ""
            ...
            if not vin or len(vin) != 17 or not re.match(r"^[A-Z0-9]{17}$", vin):
                continue
    ...
    logger.debug(f"Built assets index with {len(assets)} valid driver/VIN pairs")
    return assets


def top_matches_for_name(name: str, assets: List[Tuple[str, str]], k: int = 5) -> List[Tuple[str, str, int]]:
    """
...
def shortlist_for_group_title(title: str, assets: List[Tuple[str, str]], k_each: int = 3) -> List[Tuple[str, str, int]]:
    """
    ...
    for name in extracted_names:
        ...
        logger.debug(f"Top matches for '{name}': {[(d, v, s) for d, v, s in matches[:3]]}")
    ...
    logger.info(f"Generated shortlist with {len(shortlist)} candidates, top score: {shortlist[0][2] if shortlist else 0}")
    return shortlist
