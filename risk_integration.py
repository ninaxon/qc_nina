...
class RiskDetectionMixin:
    ...
    def init_risk_detection(self):
        ...
        if not RISK_DETECTION_AVAILABLE:
            logger.warning(
                "Risk detection modules not available - cargo theft monitoring disabled")
        ...
        try:
            ...
            if ETA_SERVICE_AVAILABLE and hasattr(self.config, 'ORS_API_KEY'):
                ...
                logger.warning(
                    "ETA service not available - late notifications disabled")
            ...
            self.risk_check_interval = getattr(
                self.config, 'RISK_CHECK_INTERVAL', 300)
            self.assets_update_interval = getattr(
                self.config, 'ASSETS_UPDATE_INTERVAL', 3600)
            self.enable_risk_monitoring = getattr(
                self.config, 'ENABLE_RISK_MONITORING', True)
            self.enable_asset_updates = getattr(
                self.config, 'ENABLE_ASSET_SHEET_UPDATES', True)

            # Track last assets update to ensure hourly intervals
            self.last_assets_update = None

            # ETA alerting settings
            self.send_qc_late_alerts = getattr(
                self.config, 'SEND_QC_LATE_ALERTS', True)
            self.eta_grace_minutes = getattr(
                self.config, 'ETA_GRACE_MINUTES', 10)
            self.risk_require_late = getattr(
                self.config, 'RISK_REQUIRE_LATE', False)

            # Alert muting system
            self.mute_store = {}  # In production, use Redis or persistent storage

            logger.info(
                f"Risk detection initialized - QC Chat: {self.qc_chat_id}, MGMT Chat: {self.mgmt_chat_id}")
            logger.info(
                f"Risk zones loaded: {len(self.risk_detector.risk_zones)}")
    ...
    def schedule_risk_monitoring(self, context):
        ...
        if not RISK_DETECTION_AVAILABLE or not getattr(
                self, 'enable_risk_monitoring', False):
        ...
        try:
            ...
            logger.info(
                f"Scheduled enhanced risk monitoring every {self.risk_check_interval}s")
            ...
            logger.info(
                f"Scheduled assets location updates every {self.assets_update_interval}s")
        ...
        try:
            ...
            if hasattr(self.google_integration, 'sync_active_loads_to_assets'):
                try:
                    ...
                    if updates > 0:
                        logger.info(
                            f"Synced {updates} QC Panel updates to assets")
                    else:
            ...
            # Update fleet_status sheet if enabled (lightweight, keep in risk
            # monitoring)
            if self.enable_asset_updates and trucks:
                try:
                    if hasattr(
                            self.google_integration,
                            'update_fleet_status_sheet'):
                        self.google_integration.update_fleet_status_sheet(
                            trucks)
            ...
            for truck in trucks:
                if not all([truck.get('vin'), truck.get('lat'),
                           truck.get('lng')]):
                    continue
                ...
                # Get active load status from QC Panel data (by driver name)
                active = None
                if hasattr(
                        self.google_integration,
                        'get_active_load_status_for_driver'):
                    active = self.google_integration.get_active_load_status_for_driver(
                        driver_name)
                ...
                if (self.send_qc_late_alerts and driver_data["del_address"] and
                    ...
                    eta_info = self.eta_service.eta_from_now(
                        lat, lng, driver_data["del_address"])
                    if eta_info:
                        ...
                        if is_late and not self.is_muted(
                                f"ACK_LATE_DEL:{vin}:{driver_data['load_id']}"):
                ...
                if (self.send_qc_late_alerts and driver_data["pu_address"] and
                    ...
                    eta_info = self.eta_service.eta_from_now(
                        lat, lng, driver_data["pu_address"])
                    if eta_info:
                        is_late, diff_min = self.eta_service.is_late(
                            eta_info["eta_utc"],
                            driver_data["pu_appt"],
                        ...
                        if is_late and not self.is_muted(
                                f"ACK_LATE_PU:{vin}:{driver_data['load_id']}"):
                ...
                if hasattr(
                        self,
                        'is_alert_acknowledged') and self.is_alert_acknowledged(vin):
                    continue
            ...
            if alerts_sent > 0 or eta_alerts_sent > 0:
                logger.warning(
                    f"Enhanced monitoring: {alerts_sent} risk alerts, {eta_alerts_sent} ETA alerts, {drivers_checked} drivers checked")
            else:
                logger.debug(
                    f"Enhanced monitoring: No alerts, {drivers_checked} drivers checked")
    ...
    async def _assets_update_callback(self, context):
        ...
        try:
            logger.info(
                "🔄 Starting hourly assets sheet location/timestamp updates...")

            # Check if enough time has passed since last update (prevent
            # restart spam)
            now = datetime.utcnow()
            if self.last_assets_update:
                time_since_last = (
                    now - self.last_assets_update).total_seconds()
                min_interval = self.assets_update_interval - 300  # Allow 5min tolerance

                if time_since_last < min_interval:
                    logger.info(
                        f"⏭️ Skipping assets update - only {time_since_last:.0f}s since last update (minimum: {min_interval}s)")
                    return
            ...
            if hasattr(
                    self.google_integration,
                    'update_assets_with_current_data'):
                ...
                if "error" not in result:
                    ...
                    if assets_updated > 0:
                        logger.info(
                            f"✅ Updated {assets_updated}/{trucks_processed} assets with {field_updates} field updates from TMS")
                        if new_trucks > 0:
                            logger.info(
                                f"📋 Found {new_trucks} new trucks in TMS not in assets sheet")
                    else:
                        logger.info(
                            "ℹ️ No asset location updates needed - all data current")
                    ...
                    logger.error(
                        f"❌ Assets location update failed: {result['error']}")

            else:
                logger.error(
                    "❌ Method update_assets_with_current_data not found on google_integration")
    ...
    async def _send_qc_late_alert_del(
            self,
            context,
            destinations: list,
            vin: str,
        ...
        try:
            ...
            for chat_id in destinations:
                try:
                    ...
                    logger.error(
                        f"Failed to send DEL late alert to {chat_id}: {e}")

            logger.info(
                f"Sent DEL late alert for {vin} to {success_count}/{len(destinations)} chats")
    ...
    async def _send_qc_late_alert_pu(
            self,
            context,
            destinations: list,
            vin: str,
        ...
        try:
            ...
            for chat_id in destinations:
                try:
                    ...
                    logger.error(
                        f"Failed to send PU late alert to {chat_id}: {e}")

            logger.info(
                f"Sent PU late alert for {vin} to {success_count}/{len(destinations)} chats")
            return success_count > 0
    ...
    async def _send_risk_alert_with_ack(self, context, alert) -> bool:
        ...
        try:
            ...
            keyboard = [[InlineKeyboardButton(
                "✅ Acknowledged", callback_data=f"ack_alert_{alert.alert_id}")]]
            ...
            if getattr(self, 'mgmt_chat_id', None):
                try:
                    ...
                    if isinstance(
                            self.mgmt_chat_id,
                            str) and ',' in self.mgmt_chat_id:
                        mgmt_ids = [
                            int(x.strip()) for x in self.mgmt_chat_id.split(',') if x.strip()]
                    ...
                    logger.error(
                        f"Failed to send risk alert to management: {e}")

            if success_count > 0:
                logger.info(
                    f"Risk alert sent for {alert.vin} to {success_count} chats")
                return True
            else:
                logger.warning(
                    "Risk alert generated but no chat IDs configured")
                return False
    ...
    async def handle_risk_alert_callback(self, update, context):
        ...
        try:
            if callback_data.startswith("ack_alert_"):
                ...
                keyboard = [[InlineKeyboardButton(
                    "📞 Contact Driver", callback_data=f"contact_driver_{vin}")]]
    ...
    async def _handle_contact_driver(self, query, vin: str):
        ...
        try:
            ...
            if hasattr(
                    self,
                    'google_integration') and hasattr(
                    self.google_integration,
                    'get_driver_contact_info_by_vin'):
                try:
                    name, phone = self.google_integration.get_driver_contact_info_by_vin(
                        vin)
                    if name:
                        driver_name = name
                    if phone:
