...
class HealthCheckHandler(BaseHTTPRequestHandler):
    ...
    def _handle_readiness_check(self):
        ...
        if not self.health_monitor:
            self._send_response(
                503, {
                    'status': 'not ready', 'reason': 'Health monitor not available'})
            return
    ...
    def _check_google_sheets(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if os.path.exists(self.config.SHEETS_SERVICE_ACCOUNT_FILE):
                return {
                    'healthy': True,
                    'message': 'Google Sheets credentials available'}
            else:
                return {
                    'healthy': False,
            ...
            return {
                'healthy': False,
                'message': f'Google Sheets check failed: {e}'}
    ...
    def _check_tms_service(self) -> Dict[str, Any]:
        ...
        try:
            if (self.config.TMS_API_URL and
                ...
                return {
                    'healthy': False,
                    'message': 'TMS service not fully configured'}
        except Exception as e:
        ...
        try:
            ...
            def handler_factory(*args, **kwargs):
                return HealthCheckHandler(
                    *args, health_monitor=self.health_monitor, **kwargs)
            ...
            logger.info(
                f"Health endpoints: http://0.0.0.0:{self.port}/health, /ready, /metrics")
