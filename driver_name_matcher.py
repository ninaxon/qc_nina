...
class DriverNameMatcher:
    ...
    def _build_driver_vin_cache(self):
        ...
        try:
            # FORCE: Always get the assets worksheet directly by name to avoid
            # any confusion
            assets_sheet_name = self.google_integration.config.SPREADSHEET_ASSETS
            logger.info(
                f"DriverNameMatcher: Forcing direct access to '{assets_sheet_name}' worksheet")

            try:
                worksheet = self.google_integration.spreadsheet.worksheet(
                    assets_sheet_name)
                logger.info(
                    f"✅ Successfully accessed worksheet: '{worksheet.title}'")
            except Exception as ws_error:
                logger.error(
                    f"❌ Failed to access worksheet '{assets_sheet_name}': {ws_error}")
                raise
            ...
            if driver_col_idx is None or vin_col_idx is None:
                logger.error(
                    f"Could not find driver name or VIN columns. Headers: {headers}")
                return

            logger.info(
                f"Found driver name column at index {driver_col_idx}, VIN column at index {vin_col_idx}")
            ...
            for i, row in enumerate(all_values[1:], 2):  # Skip header
                if len(row) > max(driver_col_idx, vin_col_idx):
                    ...
                    if driver_name and vin and len(
                            driver_name) > 2 and len(vin) >= 10:
                        # Store normalized driver name -> VIN mapping
                        normalized_name = self._normalize_driver_name(
                            driver_name)
                        ...
                        # Store first name and last name separately for partial
                        # matching
            ...
            logger.info(
                f"Built driver-VIN cache with {len(self.driver_vin_cache)} entries")
    ...
    def parse_driver_name_from_group_title(
            self, group_title: str) -> Optional[str]:
        """
    ...
    def find_vin_for_driver(self, driver_name: str) -> Optional[str]:
        ...
        for cached_name, vin in self.driver_vin_cache.items():
            ...
            if (driver_name_lower in cached_lower or
                ...
                logger.info(
                    f"Partial match: '{driver_name}' -> '{cached_name}'")
                return vin
        ...
        if self.driver_vin_cache:
            ...
            if best_match and best_match[1] >= threshold:
                ...
                logger.info(
                    f"Fuzzy match: '{driver_name}' -> '{matched_name}' (confidence: {best_match[1]}%)")
                return self.driver_vin_cache[matched_name]

        return None

    async def auto_register_group_with_vin(
            self, group_id: int, group_title: str, owner_user_id: Optional[int] = None) -> Dict[str, any]:
        """
        ...
        try:
            ...
            if not vin:
                return {
                    ...
                    "group_title": group_title,
                    # Show first 10
                    "available_drivers": list(self.driver_vin_cache.keys())[:10]
                }
    ...
    def _get_parsing_suggestions(self, group_title: str) -> List[str]:
        ...
        if "Truck_" in group_title:
            suggestions.append(
                "Group title contains 'Truck_' - ensure driver name is before this")

        if "Phone:" in group_title:
            suggestions.append(
                "Group title contains 'Phone:' - ensure driver name is before this")

        # Suggest format
        suggestions.append(
            "Recommended format: 'ID - Code - Driver Name - (Code) - Truck_XXX'")

        return suggestions

    async def batch_auto_register_groups(
            self, groups_data: List[Dict]) -> Dict[str, any]:
        """
        ...
        for group_data in groups_data:
            ...
            if not group_id or not title:
                results["errors"].append(
                    f"Missing group_id or title: {group_data}")
                results["failed"] += 1
                continue
            ...
            if result["success"]:
                ...
                logger.info(
                    f"Auto-registered group {group_id}: {result['driver_name']} -> {result['vin']}")
            else:
                results["errors"].append(
