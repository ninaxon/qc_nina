...
def setup_logging(config: Config):
    ...
    risk_handler.addFilter(lambda record: 'CARGO THEFT RISK' in record.getMessage())
    ...
    scheduler_handler.addFilter(lambda record: any(keyword in record.getMessage() 
                                                 for keyword in ['Enhanced update', 'Rate limited', 'Scheduled', 'Stats']))
    ...
    qc_handler.addFilter(lambda record: any(keyword in record.getMessage() 
                                           for keyword in ['QC Panel', 'sync', 'active loads', 'assets']))
    ...
    eta_handler.addFilter(lambda record: any(keyword in record.getMessage() 
                                            for keyword in ['Late Alert', 'ETA', 'ACK_LATE']))
    ...
    logger.info(f"Log files: {main_log}, {error_log}, {risk_log}, {scheduler_log}, {qc_panel_log}, {eta_alerts_log}")
...
def signal_handler(signum, frame):
    ...
    if enhanced_bot_instance:
        try:
            logger.info("Stopping enhanced bot sessions and risk monitoring...")
            if hasattr(enhanced_bot_instance, 'sessions'):
                ...
                for chat_id, session in enhanced_bot_instance.sessions.items():
                    if session.auto_refresh_job_name:
                        enhanced_bot_instance._cancel_job(chat_id, session.auto_refresh_job_name)
        ...
        async def test_app():
            try:
                ...
                if enhanced_bot and hasattr(enhanced_bot, 'risk_detector'):
                    ...
                    print(f"✅ Cargo theft risk detection: {risk_zones} zones loaded")
                    print(f"✅ Risk monitoring: {'Enabled' if enhanced_bot.enable_risk_monitoring else 'Disabled'}")
                else:
                    print(f"⚠️  Risk detection not available")
                
                # Test QC Panel integration
                if hasattr(enhanced_bot, 'google_integration') and config.QC_PANEL_SPREADSHEET_ID:
                    try:
                        active_loads = enhanced_bot.google_integration.get_active_load_map()
                        print(f"✅ QC Panel integration: {len(active_loads)} active loads found")
                    except Exception as e:
                ...
                # Test ETA service
                if hasattr(enhanced_bot, 'eta_service') and ETA_SERVICE_AVAILABLE:
                    print(f"✅ ETA alerting service: Available")
                    print(f"✅ Late notification grace: {config.ETA_GRACE_MINUTES} minutes")
                else:
...
def test_risk_detection_safe(config: Config) -> bool:
    ...
    try:
        ...
        if zone_count > 0:
            ...
            test_passed = True
            risk_level_values = {'LOW': 0, 'MODERATE': 1, 'HIGH': 2, 'CRITICAL': 3}
            
            for location in test_locations:
                try:
                    risk_level, zone = detector.check_location_risk(location['lat'], location['lng'])
                    ...
                    expected_min_level = risk_level_values.get(location['expected_min'], 0)
                    
                    if detected_level >= expected_min_level:
                        status = "✅"
                    else:
                        status = "⚠️"
                        # Don't fail the test for minor discrepancies, just warn
                    
                    print(f"   {status} {location['name']}: {risk_level.value} ({zone_name})")
            ...
            try:
                ...
                if isinstance(stats, dict) and 'total_zones' in stats:
                    print(f"   ✅ Zone statistics: {stats['total_zones']} zones loaded")
                else:
...
def test_qc_panel_integration(config: Config) -> bool:
    ...
    try:
        ...
        if active_loads:
            ...
            print(f"✅ Sample load: {sample_load.get('load_id', 'N/A')} - {sample_load.get('del_status', 'N/A')}")
...
def run_tests(config: Config) -> bool:
    ...
    try:
        ...
        print(f"✅ Group location updates: {scheduling_config['group_location_interval']}s")
        print(f"✅ Live tracking interval: {scheduling_config['live_tracking_interval']}s")
        print(f"✅ Auto-start enabled: {scheduling_config['auto_start_enabled']}")
        print(f"✅ Max live sessions: {scheduling_config['max_live_sessions']}")
        print(f"✅ Max group sessions: {scheduling_config['max_group_sessions']}")
...
async def run_enhanced_bot(config: Config):
    ...
    try:
        ...
        logger.info("Building enhanced Telegram application with QC Panel sync & ETA alerting...")
        ...
        if enhanced_bot_instance:
            logger.info(f"Enhanced bot instance found: {type(enhanced_bot_instance).__name__}")
            
            if hasattr(enhanced_bot_instance, 'risk_detector'):
                risk_zones = len(enhanced_bot_instance.risk_detector.risk_zones)
                logger.info(f"Risk detection integrated: {risk_zones} zones loaded")
            else:
        ...
        if SCHEDULER_AVAILABLE:
            try:
                ...
                logger.info(f"Scheduler configuration: {config.GROUP_LOCATION_INTERVAL}s interval, {config.LIVE_TRACKING_INTERVAL}s refresh")
                
            except Exception as e:
                logger.error(f"Failed to initialize GroupUpdateScheduler: {e}")
                logger.warning("Continuing without scheduler - bot will still work")
                scheduler_instance = None
        else:
            logger.warning("GroupUpdateScheduler not available")
        
        # Initialize risk monitoring with QC Panel sync
        if enhanced_bot_instance and hasattr(enhanced_bot_instance, 'schedule_risk_monitoring'):
            try:
                logger.info("Initializing enhanced cargo theft risk monitoring with QC Panel sync...")
                enhanced_bot_instance.schedule_risk_monitoring(app_instance)
                logger.info("✅ Enhanced cargo theft risk monitoring scheduled successfully")
                
                if hasattr(enhanced_bot_instance, 'risk_detector'):
                    risk_zones = len(enhanced_bot_instance.risk_detector.risk_zones)
                    check_interval = enhanced_bot_instance.risk_check_interval
                    logger.info(f"Risk monitoring active: {risk_zones} zones, {check_interval}s interval")
                ...
                logger.info(f"QC Panel sync: {'✅ Enabled' if qc_panel_enabled else '❌ Disabled'}")
                logger.info(f"ETA late alerts: {'✅ Enabled' if eta_alerts_enabled else '❌ Disabled'}")
        ...
        print(f"   📍 Group updates every {config.GROUP_LOCATION_INTERVAL//60} min (distributed)")
        print(f"   🎯 Max 12 concurrent sends (prevents rate limits)")
        print(f"   🎲 0-15s jitter (spreads 300 groups across 15+ minutes)")
        print(f"   🔄 Silent data refresh every {config.LIVE_TRACKING_INTERVAL//60} minutes")
        ...
        if config.QC_PANEL_SPREADSHEET_ID:
            ...
            print(f"   ⚠️ Statuses watched: {config.RISK_MONITOR_DEL_STATUSES}")
        else:
        ...
        if config.SEND_QC_LATE_ALERTS and ETA_SERVICE_AVAILABLE:
            ...
            print(f"   📧 QC notifications: {'✅ Enabled' if qc_configured else '❌ Not configured'}")
            print(f"   📧 Management notifications: {'✅ Enabled' if mgmt_configured else '❌ Not configured'}")
        else:
            print(f"\n🚨 ETA Late Notifications: ❌ Disabled")
        
        # Display risk monitoring status
        if enhanced_bot_instance and hasattr(enhanced_bot_instance, 'risk_detector'):
            risk_zones = len(enhanced_bot_instance.risk_detector.risk_zones)
            ...
            print(f"   🚨 Risk alerts: {'ACTIVE' if risk_enabled and qc_configured else 'INACTIVE'}")
            print(f"   ⏱️ Check interval: {enhanced_bot_instance.risk_check_interval//60} minutes")
            print(f"   📞 QC alerts: {'Enabled' if qc_configured else 'Not configured'}")
            print(f"   🎯 Transit load gating: {'Enabled' if config.QC_PANEL_SPREADSHEET_ID else 'Disabled'}")
            
            if risk_enabled and qc_configured:
                print("   ✅ Full risk protection active")
        
        logger.info("Enhanced bot ready for production operation with QC Panel sync")
        
        print(f"\n🚀 Starting enhanced bot for production scale...")
        print(f"📍 Group distribution: {config.GROUP_LOCATION_INTERVAL}s with jitter")
        print(f"🔢 Concurrent limit: 12 sends max")
        print(f"📊 Statistics: Use /scheduler_stats command")
        print(f"📋 QC Panel: {'Auto-sync enabled' if config.QC_PANEL_SPREADSHEET_ID else 'Not configured'}")
        print(f"🚨 ETA Alerts: {'Enabled' if config.SEND_QC_LATE_ALERTS else 'Disabled'}")
        ...
        for attempt in range(max_retries):
            try:
                print(f"🔌 Attempting to connect to Telegram API (attempt {attempt + 1}/{max_retries})...")
                ...
                if "ConnectError" in str(e) or "NetworkError" in str(e):
                    if attempt < max_retries - 1:
                        ...
                        print("   • Try running 'curl -X GET \"https://api.telegram.org/bot<YOUR_TOKEN>/getMe\"'")
                        raise
                else:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Alternative idle method failed: {fallback_error}")
                try:
                    while True:
        ...
        logger.info("Received KeyboardInterrupt, initiating enhanced shutdown...")
        raise
    except Exception as e:
...
def main():
    ...
    try:
        ...
        logger.info(f"Group location interval: {config.GROUP_LOCATION_INTERVAL}s ({config.GROUP_LOCATION_INTERVAL//60} min)")
        logger.info(f"Enhanced scheduler: {'Available' if SCHEDULER_AVAILABLE else 'Not available'}")
        logger.info(f"Risk detection: {'Available' if RISK_DETECTION_AVAILABLE else 'Not available'}")
        logger.info(f"ETA service: {'Available' if ETA_SERVICE_AVAILABLE else 'Not available'}")
        logger.info(f"QC Panel sync: {'Enabled' if config.QC_PANEL_SPREADSHEET_ID else 'Disabled'}")
        logger.info(f"ETA late alerts: {'Enabled' if config.SEND_QC_LATE_ALERTS else 'Disabled'}")
