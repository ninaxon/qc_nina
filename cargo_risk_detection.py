...
class CargoTheftRiskDetector:
    ...
    def __init__(self, config, tms_integration=None):
        ...
        self.alert_cooldown = timedelta(minutes=getattr(config, 'ALERT_COOLDOWN_MINUTES', 30))
        
        # Risk detection settings
        self.min_stop_duration = timedelta(minutes=getattr(config, 'MIN_STOP_DURATION_MINUTES', 10))
        self.max_speed_threshold = getattr(config, 'MAX_SPEED_THRESHOLD_MPH', 2.0)
        ...
        logger.info(f"Simplified cargo theft risk detector initialized with {len(self.risk_zones)} zones")
    ...
    def _initialize_risk_zones(self):
        ...
        self.risk_zones.append(RiskZone(
            ...
            min_lat=33.4, max_lat=34.4,
            min_lng=-118.8, max_lng=-117.1,
        ...
        self.risk_zones.append(RiskZone(
            ...
            risk_level=RiskLevel.CRITICAL,
            min_lat=34.8, max_lat=35.4,
            min_lng=-90.4, max_lng=-89.6,
        ...
        logger.info(f"Initialized {len(self.risk_zones)} simplified cargo theft risk zones")
        logger.info(f"Zone breakdown: {sum(1 for z in self.risk_zones if z.risk_level == RiskLevel.CRITICAL)} Critical, "
    ...
    def check_location_risk(self, lat: float, lng: float) -> Tuple[RiskLevel, Optional[RiskZone]]:
    ...
    def _get_driver_contact_info(self, vin: str) -> Tuple[str, str]:
        ...
        try:
            if self.google_integration and hasattr(self.google_integration, 'get_driver_contact_info_by_vin'):
                # Use the enhanced method that looks up by VIN in assets worksheet
                driver_name, phone = self.google_integration.get_driver_contact_info_by_vin(vin)
    ...
    def update_driver_state(self, driver_data: Dict[str, Any]) -> Optional[RiskAlert]:
        """
        ...
        if is_currently_stopped:
            ...
            if (driver_state['stop_start_time'] and 
                risk_level in [RiskLevel.HIGH, RiskLevel.CRITICAL] and
                current_time - driver_state['stop_start_time'] >= self.min_stop_duration):
                
                # Check alert cooldown
                alert_key = f"{vin}_{risk_zone.name}"
                if (alert_key not in self.recent_alerts or 
                    current_time - self.recent_alerts[alert_key] >= self.alert_cooldown):
                    
                    # Generate alert
                    alert = self._create_risk_alert(driver_data, risk_level, risk_zone, 
                                                  current_time - driver_state['stop_start_time'])
                    
                    # Update alert tracking
                    self.recent_alerts[alert_key] = current_time
                    
                    logger.warning(f"CARGO THEFT RISK ALERT: {vin} stopped in {risk_zone.name}")
    ...
    def _create_risk_alert(self, driver_data: Dict[str, Any], risk_level: RiskLevel, 
                          risk_zone: RiskZone, stop_duration: timedelta) -> RiskAlert:
    ...
    def format_risk_alert_message(self, alert: RiskAlert) -> str:
        ...
        # COMPACT FORMAT - Only essential information
        message = (
            ...
            f"🔗 [View on Map]({alert.google_maps_link})\n\n"
    ...
    def cleanup_old_states(self, hours: int = 24):
        ...
        cutoff_time = datetime.now(pytz.timezone('America/New_York')) - timedelta(hours=hours)
        ...
        logger.info(f"Cleaned up {len(vins_to_remove)} old driver states and {len(alerts_to_remove)} old alerts")
...
def test_simplified_risk_detection():
    ...
    test_locations = [
        ...
        {'lat': 34.0522, 'lng': -118.2437, 'expected': RiskLevel.CRITICAL, 'name': 'Los Angeles'},
        # Dallas - CRITICAL
        {'lat': 32.7767, 'lng': -96.7970, 'expected': RiskLevel.CRITICAL, 'name': 'Dallas'},
        # Atlanta - HIGH
        {'lat': 33.7490, 'lng': -84.3880, 'expected': RiskLevel.HIGH, 'name': 'Atlanta'},
        # Denver - MODERATE
        {'lat': 39.7392, 'lng': -104.9903, 'expected': RiskLevel.MODERATE, 'name': 'Denver'},
        # Rural Montana - LOW
        {'lat': 47.0527, 'lng': -109.6333, 'expected': RiskLevel.LOW, 'name': 'Rural Montana'}
    ]
    ...
    for i, location in enumerate(test_locations):
        ...
        print(f"Test {i+1} {status}: {location['name']} -> Expected: {expected}, Got: {actual} ({zone_name})")
