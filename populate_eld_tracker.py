...
def populate_eld_tracker():
    ...
    try:
        ...
        print(f"  VIN: Column {vin_col} ({assets_headers[vin_col] if vin_col is not None else 'Not found'})")
        print(f"  Driver: Column {driver_col} ({assets_headers[driver_col] if driver_col is not None else 'Not found'})")
        print(f"  Gateway: Column {gateway_col} ({assets_headers[gateway_col] if gateway_col is not None else 'Not found'})")
        ...
        for row in assets_rows:
            if len(row) > vin_col and row[vin_col].strip():
                ...
                if vin:  # Only include rows with valid VINs
                    ...
                    gateway = row[gateway_col] if gateway_col is not None and len(row) > gateway_col else ""
                    serial = row[serial_col] if serial_col is not None and len(row) > serial_col else ""
                    driver = row[driver_col] if driver_col is not None and len(row) > driver_col else ""
        ...
        if eld_data:
            ...
            print(f"✅ Successfully populated ELD_tracker with {len(eld_data)} records")
