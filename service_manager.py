    ...
    def start(self, daemon: bool = True) -> bool:
        ...
        try:
            ...
            if self.is_running():
                ...
                print(f"✅ Service '{self.service_name}' started successfully (PID: {process.pid})")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Asset Tracking Bot Service Manager")
    parser.add_argument("action", choices=["start", "stop", "restart", "status", "logs", "cleanup"],
    ...
    if args.action == "start":
        ...
        if 'last_status' in status:
            ...
            print(f"   Last Status: {last.get('status', 'N/A')} ({last.get('timestamp', 'N/A')})")
