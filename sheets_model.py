...
class SheetsModelManager:
    ...
    def _find_header_column(self, header_map: Dict[str, int], target: str) -> Optional[int]:
    ...
    def _ensure_worksheet_headers(self, worksheet, schema: SheetSchema) -> bool:
        ...
        try:
            ...
            if not current_headers or len(current_headers) < len(schema.headers):
        ...
        try:
            ...
            for asset in tms_assets:
                ...
                if vin in vin_to_row:
                    ...
                    driver_col = self._find_header_column(header_map, 'Driver Name')
                    ...
                    last_seen_col = self._find_header_column(header_map, 'Last Seen UTC')
                    if last_seen_col is not None:
                        updates[last_seen_col] = current_time_utc.isoformat()
                    
                    source_col = self._find_header_column(header_map, 'Last Sync Source')
            ...
            if batch_updates:
                ...
                for i in range(0, len(batch_updates), chunk_size):
                    ...
                    try:
                        ...
                        logger.error(f"Batch update failed for assets chunk {i//chunk_size}: {e}")
                ...
                try:
                    ...
                    for i in range(0, len(new_rows), chunk_size):
                        ...
                        for row in chunk:
                            ...
                            await asyncio.sleep(0.05)  # Small delay between appends
                except Exception as e:
                    logger.error(f"Failed to append new assets: {e}")
            
            self.metrics['assets_upserted'] += upserted_count
            logger.info(f"Assets sync complete: {upserted_count} VINs processed")
    ...
    async def register_or_update_group(self, group_id: int, title: str, vin: str, 
        ...
        try:
            ...
            if existing_row_num:
                ...
                logger.info(f"Updated group registration: {group_id} -> {vin_upper}")
            else:
                # Add new
        ...
        try:
            ...
            for i, row in enumerate(all_data[1:], start=2):
                if (len(row) > group_id_col and 
                    ...
                    logger.info(f"Updated group {group_id} title to: {new_title}")
                    break
            ...
            if batch_updates:
                ...
                for i in range(0, len(batch_updates), chunk_size):
                    ...
                    try:
                        ...
                        logger.error(f"ELD_tracker batch update failed for chunk {i//chunk_size}: {e}")
        ...
        try:
            ...
            if new_rows:
                try:
                    ...
                    logger.error(f"Failed to append new fleet_status rows: {e}")
    ...
    def append_location_logs(self, events: Iterable[dict]) -> int:
        ...
        try:
            ...
            old_keys = [k for k, v in self.location_logs_outbox.items() if v < cutoff]
            for key in old_keys:
            ...
            for event in events:
                ...
                ts_bucket = int(event.get('ts_utc_timestamp', time.time()) // 300)  # 5-min buckets
                outbox_key = f"{ts_bucket}|{event.get('VIN', '')}|{event.get('event_type', '')}"
                
                if outbox_key in self.location_logs_outbox:
                    logger.debug(f"Skipping duplicate location log: {outbox_key}")
                    continue
                ...
                self.location_logs_outbox[outbox_key] = datetime.now(self.utc_tz)
                appended_count += 1
    ...
    def append_dashboard_hourly(self, summary: dict) -> None:
        ...
        try:
            ...
            logger.info(f"Dashboard summary appended for {summary.get('date')}")
    ...
    def append_ack_audit(self, entry: dict) -> None:
        ...
        try:
            ...
            audit_row = [
                ...
                entry.get('stop_type', ''),                 # stop_type (PU/DEL)
            ...
            logger.info(f"ACK audit logged: {entry.get('driver_id')} - {entry.get('stop_type')}")
    ...
    def log_severe_error(self, component: str, severity: str, summary: str, 
        ...
        try:
            ...
            logger.info(f"Severe error logged: {component} - {severity} - {summary}")
    ...
    def prune_location_logs_older_than(self, days: int) -> int:
        ...
        try:
            worksheet = self._get_worksheet_safe('location_logs')
            if not worksheet:
                logger.error("Location_logs worksheet not available for pruning")
                return 0
            ...
            for i, row in enumerate(data_rows):
                if len(row) > ts_utc_col and row[ts_utc_col]:
                    try:
                        row_date = datetime.fromisoformat(row[ts_utc_col].replace('Z', '+00:00'))
                        if row_date < cutoff_date:
                            rows_to_delete.append(i + 2)  # +2 for header and 1-based
                    except ValueError:
            ...
            if rows_to_delete:
                ...
                logger.info(f"Pruned {deleted_count} location log entries older than {days} days")
