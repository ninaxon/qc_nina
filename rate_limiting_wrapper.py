...
class RateLimitedSheetsWrapper:
    ...
    def __init__(self, config=None):
        ...
        self.base_delay = getattr(
            config, 'SHEETS_EXPONENTIAL_BACKOFF_BASE', 1.0)
        self.max_delay = getattr(
            config, 'SHEETS_EXPONENTIAL_BACKOFF_MAX', 60.0)
        ...
        self.default_cache_ttl = getattr(
            config, 'SHEETS_CACHE_DEFAULT_TTL', 300)
        ...
        self.circuit_breaker_threshold = getattr(
            config, 'SHEETS_CIRCUIT_BREAKER_THRESHOLD', 10)
        self.circuit_breaker_timeout = getattr(
            config, 'SHEETS_CIRCUIT_BREAKER_TIMEOUT', 300)
        ...
        self.max_requests_per_minute = getattr(
            config, 'SHEETS_MAX_REQUESTS_PER_MINUTE', 180)
    ...
    def _store_in_cache(self, cache_key: str, data: Any, ttl: int):
        ...
        with self.cache_lock:
            ...
            logger.debug(
                f"Cached data for key: {cache_key[:8]}... (TTL: {ttl}s)")
    ...
    def _check_quota_limit(self) -> bool:
        ...
        if self.request_count >= self.max_requests_per_minute:
            ...
            logger.warning(
                f"Quota limit reached, waiting {wait_time:.1f}s for reset")
    ...
    def _execute_with_retry(
        ...
        for attempt in range(max_attempts):
            try:
                ...
                should_retry = (
                    is_rate_limit or is_connection_error) and attempt < max_attempts - 1

                if should_retry:
                    delay = self._calculate_delay(attempt)
                    error_type = "rate limit" if is_rate_limit else "connection issue"
                    logger.warning(
                        f"{error_type} on {method_name} (attempt {attempt + 1}/{max_attempts}): {e}, "
                ...
                if attempt == max_attempts - 1 or not should_retry:
                    logger.error(
                        f"Failed to execute {method_name} after {attempt + 1} attempts: {e}")
                    raise e
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        with self.cache_lock:
            ...
            return {
                ...
                'quota_reset_in': (
                    self.quota_reset_time - datetime.now()).total_seconds()}
    ...
    def __getattr__(self, name):
        ...
        def rate_limited_method(*args, **kwargs):
            ...
            if self._rate_limiter._is_cacheable_method(name):
                ...
                cache_key = self._rate_limiter._get_cache_key(
                    name, args, kwargs)
                ...
                result = self._rate_limiter._execute_with_retry(
                ...
                return self._rate_limiter._execute_with_retry(
                    attr, name, *args, **kwargs)
...
def create_rate_limited_google_integration(config):
    ...
    if integration.assets_worksheet:
        integration.assets_worksheet = rate_limiter.wrap_worksheet(
            integration.assets_worksheet)

    if integration.groups_worksheet:
        integration.groups_worksheet = rate_limiter.wrap_worksheet(
            integration.groups_worksheet)

    if integration.fleet_status_worksheet:
        integration.fleet_status_worksheet = rate_limiter.wrap_worksheet(
            integration.fleet_status_worksheet)

    if integration.dashboard_logs_worksheet:
        integration.dashboard_logs_worksheet = rate_limiter.wrap_worksheet(
            integration.dashboard_logs_worksheet)
...
def cleanup_cache_periodically(
        rate_limiter: RateLimitedSheetsWrapper,
        interval: int = 300):
    """
