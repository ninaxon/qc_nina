...
# Score threshold for auto-registration (higher threshold for automatic actions)
AUTO_REGISTER_THRESHOLD = 85


async def auto_register_vin_on_group_join(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    ...
    logger.info(f"Auto VIN registration triggered - Group: {group_id}, Title: '{safe_title}'")
    ...
    try:
        ...
        if existing_vin:
            ...
            safe_title = group_title.replace('*', '\\*').replace('_', '\\_').replace('[', '\\[').replace(']', '\\]').replace('`', '\\`') if group_title else "Unknown Group"
    ...
    # Generate shortlist
    try:
        shortlist = shortlist_for_group_title(group_title, assets_index, k_each=3)
        extracted_names = extract_names_from_title(group_title)
        logger.info(f"Auto-registration - Extracted names: {extracted_names}")
        
        if shortlist:
            logger.info(f"Auto-registration - Top matches: {[(d, v[-4:], s) for d, v, s in shortlist[:3]]}")
    ...
    if shortlist:
        auto_register_candidates = [item for item in shortlist if item[2] >= AUTO_REGISTER_THRESHOLD]
        ...
        if auto_register_candidates and len(auto_register_candidates) == 1:
            ...
            logger.info(f"Auto-registering VIN {vin} for group {group_id} with confidence {score}%")
            
            # Double-check that VIN isn't already registered (race condition protection)
            existing_vin_check = await get_existing_group_vin(group_id, context)
            if existing_vin_check:
                logger.info(f"VIN already registered during auto-registration attempt: {existing_vin_check}")
                return
            ...
            try:
                ...
                if success:
                    ...
                    safe_driver = driver_name.replace('*', '\\*').replace('_', '\\_').replace('[', '\\[').replace(']', '\\]').replace('`', '\\`') if driver_name else "Unknown Driver"
                    ...
                    try:
                        ...
                        logger.info(f"Successfully auto-registered VIN {vin.upper()} for group {group_id}")
                        return  # Success!
                        
                    except Exception as e:
                        logger.error(f"Error sending auto-registration success message: {e}")
                        return
                        
                else:
                    logger.warning(f"Auto-registration failed for VIN {vin}, will not show manual options")
    ...
    logger.info(f"No auto-registration performed for group {group_id} - confidence too low or multiple matches")


async def suggest_vin_on_group_join(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    ...
    # Log the trigger (redact phone if present)
    safe_title = redact_phone(group_title)
    logger.info(f"VIN suggestion triggered - Group: {group_id}, Title: '{safe_title}'")
    ...
    try:
        ...
        assets_index = build_assets_index(assets_data, driver_col=3, vin_col=4)  # D=3, E=4 (0-indexed)
        logger.debug(f"Built assets index with {len(assets_index)} entries")
    ...
    try:
        ...
        if shortlist:
            logger.info(f"Top matches: {[(d, v[-4:], s) for d, v, s in shortlist[:3]]}")
    ...
    if not shortlist:
        ...
        escaped_title = safe_title.replace('*', '\\*').replace('_', '\\_').replace('[', '\\[').replace(']', '\\]').replace('`', '\\`')
        ...
        keyboard = [[InlineKeyboardButton("🔍 Manual Search", callback_data=CB_MANUAL_SEARCH)]]
        ...
        if auto_register_candidates and len(auto_register_candidates) == 1:
            ...
            existing_vin_check = await get_existing_group_vin(group_id, context)
            if existing_vin_check:
                logger.info(f"VIN already registered during manual VIN suggestion attempt: {existing_vin_check}")
                # Return early, but don't show error - fall through to normal suggestion logic
            else:
                try:
                    success = await save_group_vin(group_id, vin.upper(), context, group_title)
                    if success:
                        # Escape special Markdown characters in group title and driver name
                        safe_title = group_title.replace('*', '\\*').replace('_', '\\_').replace('[', '\\[').replace(']', '\\]').replace('`', '\\`') if group_title else "Unknown Group"
                        safe_driver = driver_name.replace('*', '\\*').replace('_', '\\_').replace('[', '\\[').replace(']', '\\]').replace('`', '\\`') if driver_name else "Unknown Driver"
                        ...
                        logger.warning(f"Auto-registration failed for VIN {vin}, falling back to manual selection")
        ...
        high_conf = [item for item in shortlist if item[2] >= CONFIDENCE_THRESHOLD]
        low_conf = [item for item in shortlist if item[2] < CONFIDENCE_THRESHOLD]
        ...
        if high_conf:
            ...
            for driver_name, vin, score in high_conf[:6]:  # Max 6 high-confidence
                # Truncate long names for button display
                display_name = driver_name[:15] + "..." if len(driver_name) > 15 else driver_name
                button_text = f"✅ {score}% • {display_name} • {vin[:8]}..."
                keyboard_items.append([InlineKeyboardButton(button_text, callback_data=f"{CB_VIN_SELECTED}|{vin}")])
            ...
            for driver_name, vin, score in shortlist[:3]:  # Show top 3 even if low confidence
                display_name = driver_name[:15] + "..." if len(driver_name) > 15 else driver_name
                button_text = f"⚠️ {score}% • {display_name} • {vin[:8]}..."
                keyboard_items.append([InlineKeyboardButton(button_text, callback_data=f"{CB_VIN_SELECTED}|{vin}")])
        
        # Add manual search option
        keyboard_items.append([InlineKeyboardButton("🔍 Search Manually", callback_data=CB_MANUAL_SEARCH)])
...
async def on_vin_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"VIN selection callback: {callback_data} from user {user.id} in group {group_id}")
        ...
        if callback_data == CB_MANUAL_SEARCH:
            ...
            try:
                ...
                if success:
                    ...
                    safe_driver = driver_name.replace('*', '\\*').replace('_', '\\_').replace('[', '\\[').replace(']', '\\]').replace('`', '\\`') if driver_name else ""
                    driver_display = f" ({safe_driver})" if safe_driver else ""
                    
                    # Escape special Markdown characters in group title
                    safe_title = chat.title.replace('*', '\\*').replace('_', '\\_').replace('[', '\\[').replace(']', '\\]').replace('`', '\\`') if chat.title else "Unknown Group"
                    ...
                    logger.info(f"VIN {vin.upper()} successfully registered for group {group_id}")
                    
                else:
                    message = (
                        f"❌ **Registration Failed**\n\n"
                        f"Unable to save VIN to database. Please try again or contact support.\n\n"
                        f"_Error code: DB_SAVE_FAILED_"
                    )
...
async def load_assets_data(context: ContextTypes.DEFAULT_TYPE) -> Optional[List[List[str]]]:
    """
    ...
    try:
        ...
        if context.bot_data and 'assets_ws' in context.bot_data and context.bot_data['assets_ws']:
            return context.bot_data['assets_ws'].get_all_values()
        
        # Option 2: Use your existing GoogleSheetsIntegration
        if context.bot_data and 'google_integration' in context.bot_data and context.bot_data['google_integration']:
        ...
        logger.warning("Assets worksheet not found in context, attempting fresh connection")
...
async def get_existing_group_vin(group_id: int, context: ContextTypes.DEFAULT_TYPE) -> Optional[str]:
    """
...
async def save_group_vin(group_id: int, vin: str, context: ContextTypes.DEFAULT_TYPE, group_title: str = None) -> bool:
    """
    ...
    try:
        ...
        if not context.bot_data or 'google_integration' not in context.bot_data or not context.bot_data['google_integration']:
        ...
        try:
            ...
            logger.debug(f"Calling google.save_group_vin({group_id}, '{group_title}', '{vin}')")
            success = google.save_group_vin(group_id, group_title, vin)
            
            if success:
                logger.info(f"Successfully saved VIN {vin} for group {group_id}")
                return True
            else:
                logger.error(f"save_group_vin returned False for group {group_id}")
                return False
...
async def get_driver_name_for_vin(vin: str, context: ContextTypes.DEFAULT_TYPE) -> Optional[str]:
    """
    ...
    try:
        ...
        for row in assets_data[1:]:  # Skip header
            if len(row) > 4 and str(row[4]).strip().upper() == vin.upper():
                driver_name = str(row[3]).strip() if len(row) > 3 else ""  # Column D = index 3
...
def register_vin_handlers(application):
    ...
    application.add_handler(CommandHandler("setvin", suggest_vin_on_group_join))
    
    # Command handler for auto-registration trigger (for testing)
    application.add_handler(CommandHandler("autovin", auto_register_vin_on_group_join))
