...
class WorksheetMonitor:
        ...
        for worksheet_name, spec in self.worksheet_specs.items():
            try:
                ...
                if not status.is_healthy and spec.get('critical', False):
                    logger.error(
                        f"CRITICAL: {worksheet_name} worksheet is unhealthy - {status.error_message}")
                elif not status.is_healthy:
                    logger.warning(
                        f"WARNING: {worksheet_name} worksheet is unhealthy - {status.error_message}")
    ...
    async def _check_worksheet(self,
                               worksheet_name: str,
                               spec: Dict[str,
                                          Any]) -> WorksheetStatus:
        ...
        try:
            ...
            is_healthy = self._is_worksheet_healthy(
                last_update, spec['expected_interval'])
            ...
            if not is_healthy:
                if last_update:
                    age_minutes = (
                        datetime.now(
                            pytz.timezone('America/New_York')) - last_update).total_seconds() / 60
    ...
    async def _get_last_update_time(self,
                                    worksheet_name: str,
                                    worksheet,
                                    all_values: List[List[str]]) -> Optional[datetime]:
        ...
        try:
            if worksheet_name == 'assets':
                ...
                headers = [h.lower()
                           for h in all_values[0]] if all_values else []
                time_cols = []

                for i, header in enumerate(headers):
                    if any(
                        time_field in header for time_field in [
                            'sync_time',
                            'last_updated',
                            'updated_at']):
                        time_cols.append(i)
            ...
            logger.debug(
                f"Error parsing update time for {worksheet_name}: {e}")

        return None

    def _is_worksheet_healthy(
            self,
            last_update: Optional[datetime],
            expected_interval_seconds: int) -> bool:
    ...
    def generate_health_report(
            self, statuses: Dict[str, WorksheetStatus]) -> str:
        """Generate human-readable health report"""
        report_lines = []
        report_lines.append("📊 **Worksheet Health Report**")
        report_lines.append(
            f"Generated: {datetime.now(pytz.timezone('America/New_York')).strftime('%Y-%m-%d %H:%M:%S %Z')}")
        report_lines.append("")

        healthy_count = sum(
            1 for status in statuses.values() if status.is_healthy)
        total_count = len(statuses)

        report_lines.append(
            f"**Overall Status: {healthy_count}/{total_count} worksheets healthy**")
        report_lines.append("")
        ...
        for name, status in statuses.items():
            ...
            report_lines.append(
                f"{icon} **{name.title()} Worksheet** ({priority})")
            report_lines.append(f"   • Rows: {status.row_count}")
            report_lines.append(
                f"   • Expected interval: {status.expected_interval_seconds//60} min")
            report_lines.append(f"   • Update method: {status.update_method}")

            if status.last_update:
                age_minutes = (
                    datetime.now(
                        pytz.timezone('America/New_York')) - status.last_update).total_seconds() / 60
                report_lines.append(
                    f"   • Last update: {age_minutes:.1f} min ago")
            else:
                report_lines.append("   • Last update: Unknown")
        ...
        unhealthy = [
            name for name,
            status in statuses.items() if not status.is_healthy]
        if unhealthy:
            ...
            for name in unhealthy:
                ...
                if name == 'assets':
                    report_lines.append(
                        f"   • Check QC Panel sync job (every {spec['expected_interval']//60} min)")
                elif name == 'fleet_status':
                    report_lines.append(
                        "   • Check TMS integration and risk monitoring jobs")
                elif name == 'dashboard_logs':
                    report_lines.append("   • Check user interaction logging")
                elif name == 'groups':
                    report_lines.append(
                        "   • Check group registration updates")
            report_lines.append("")
