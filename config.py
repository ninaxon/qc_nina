...
class Config:
    ...
    def __init__(self):
        ...
        self.SHEETS_SERVICE_ACCOUNT_FILE = self._get_env_or_error(
            "SHEETS_SERVICE_ACCOUNT_FILE")
        self.SPREADSHEET_ID = self._get_env_or_error("SPREADSHEET_ID")
        self.SPREADSHEET_ASSETS = self._get_optional(
            "SPREADSHEET_ASSETS", "assets")
        self.SPREADSHEET_GROUPS = self._get_optional(
        ...
        # Google Maps API (Optional fallback for geocoding)
        self.GOOGLE_MAPS_API_KEY = self._get_optional(
            "GOOGLE_MAPS_API_KEY", None)
        self.GOOGLE_MAPS_TIMEOUT_SECS = self._get_optional_int(
            "GOOGLE_MAPS_TIMEOUT_SECS", 5)

        # =====================================================
        # NEW: QC PANEL INTEGRATION CONFIGURATION
        # =====================================================
        self.QC_PANEL_SPREADSHEET_ID = self._get_optional(
            "QC_PANEL_SPREADSHEET_ID", "")
        self.QC_ACTIVE_TABS = self._get_optional(
            "QC_ACTIVE_TABS", "BIDH S,BIDH D,CPWP S,CPWP D,SSOY,OTMV")
        self.RISK_MONITOR_DEL_STATUSES = self._get_optional(
            "RISK_MONITOR_DEL_STATUSES", "IN TRANSIT,TRANSIT,WILL BE LATE,AT SHIPPER,RISKY,Will be late")

        # =====================================================
        # RATE LIMITING CONFIGURATION
        # =====================================================
        self.SHEETS_RATE_LIMIT_ENABLED = self._get_optional_bool(
            "SHEETS_RATE_LIMIT_ENABLED", True)
        self.SHEETS_MAX_REQUESTS_PER_MINUTE = self._get_optional_int(
            "SHEETS_MAX_REQUESTS_PER_MINUTE", 180)
        self.SHEETS_CACHE_DEFAULT_TTL = self._get_optional_int(
            "SHEETS_CACHE_DEFAULT_TTL", 300)  # 5 minutes
        self.SHEETS_CACHE_LONG_TTL = self._get_optional_int(
            "SHEETS_CACHE_LONG_TTL", 1800)  # 30 minutes
        self.SHEETS_EXPONENTIAL_BACKOFF_BASE = self._get_optional_float(
            "SHEETS_EXPONENTIAL_BACKOFF_BASE", 1.0)
        self.SHEETS_EXPONENTIAL_BACKOFF_MAX = self._get_optional_float(
            "SHEETS_EXPONENTIAL_BACKOFF_MAX", 60.0)
        self.SHEETS_CIRCUIT_BREAKER_THRESHOLD = self._get_optional_int(
            "SHEETS_CIRCUIT_BREAKER_THRESHOLD", 10)
        self.SHEETS_CIRCUIT_BREAKER_TIMEOUT = self._get_optional_int(
            "SHEETS_CIRCUIT_BREAKER_TIMEOUT", 300)  # 5 minutes

        # =====================================================
        # GOOGLE SHEETS COLUMN MAPPING (A,B,C notation)
        # =====================================================
        self.USE_COLUMN_MAPPING = self._get_optional_bool(
            "USE_COLUMN_MAPPING", True)

        # Assets worksheet columns - customize if your sheet has different
        # layout  
        self.ASSETS_DRIVER_NAME_COL = self._get_optional(
            "ASSETS_DRIVER_NAME_COL", "A")  # Column A (First name) + B (Last name)
        self.ASSETS_VIN_COL = self._get_optional(
            "ASSETS_VIN_COL", "C")  # Column C
        self.ASSETS_LOCATION_COL = self._get_optional(
            "ASSETS_LOCATION_COL", "F")  # Column F
        self.ASSETS_LATITUDE_COL = self._get_optional(
            "ASSETS_LATITUDE_COL", "G")  # Column G
        self.ASSETS_LONGITUDE_COL = self._get_optional(
            "ASSETS_LONGITUDE_COL", "H")  # Column H
        self.ASSETS_PHONE_COL = self._get_optional(
            "ASSETS_PHONE_COL", "L")  # Column L
        ...
        self.ORS_MAX_REQUESTS_PER_MINUTE = self._get_optional_int(
            "ORS_MAX_REQUESTS_PER_MINUTE", 30)  # Conservative for mixed endpoints
        self.ORS_REQUEST_DELAY = self._get_optional_float(
            "ORS_REQUEST_DELAY", 2.0)  # 2 second minimum delay between requests
        self.ORS_ENABLE_CACHING = self._get_optional_bool(
            "ORS_ENABLE_CACHING", True)
        self.ORS_CACHE_TTL = self._get_optional_int(
            "ORS_CACHE_TTL", 3600)  # 1 hour cache

        # ETA / Alerting Configuration
        self.ETA_GRACE_MINUTES = self._get_optional_int(
            "ETA_GRACE_MINUTES", 10)
        self.SEND_QC_LATE_ALERTS = self._get_optional_bool(
            "SEND_QC_LATE_ALERTS", True)
        self.RISK_REQUIRE_LATE = self._get_optional_bool(
            "RISK_REQUIRE_LATE", False)
        ...
        self.GROUP_LOCATION_INTERVAL = self._get_optional_int(
            "GROUP_LOCATION_INTERVAL", 3600)  # 1 hour

        # Silent Data Refresh (8-minute - Updates TMS data to assets sheet)
        self.LIVE_TRACKING_INTERVAL = self._get_optional_int(
            "LIVE_TRACKING_INTERVAL", 480)  # 8 minutes

        # Legacy support (maps to LIVE_TRACKING_INTERVAL for backward
        # compatibility)
        self.LIVE_UPDATE_INTERVAL = self.LIVE_TRACKING_INTERVAL

        # Group Auto-Update Settings
        self.ENABLE_GROUP_LOCATION_UPDATES = self._get_optional_bool(
            "ENABLE_GROUP_LOCATION_UPDATES", True)
        self.ENABLE_LIVE_ETA_TRACKING = self._get_optional_bool(
            "ENABLE_LIVE_ETA_TRACKING", True)
        self.AUTO_START_LOCATION_UPDATES = self._get_optional_bool(
            "AUTO_START_LOCATION_UPDATES", True)

        # Session Management
        self.MAX_LIVE_SESSIONS = self._get_optional_int(
            "MAX_LIVE_SESSIONS", 100)
        self.MAX_GROUP_SESSIONS = self._get_optional_int(
            "MAX_GROUP_SESSIONS", 50)
        self.SESSION_TIMEOUT_HOURS = self._get_optional_int(
        ...
        self.ENABLE_RISK_MONITORING = self._get_optional_bool(
            "ENABLE_RISK_MONITORING", True)
        self.RISK_CHECK_INTERVAL = self._get_optional_int(
            "RISK_CHECK_INTERVAL", 300)  # 5 minutes
        self.ASSETS_UPDATE_INTERVAL = self._get_optional_int(
            "ASSETS_UPDATE_INTERVAL", 3600)  # 1 hour
        ...
        self.MIN_STOP_DURATION_MINUTES = self._get_optional_int(
            "MIN_STOP_DURATION_MINUTES", 10)
        self.MAX_SPEED_THRESHOLD_MPH = self._get_optional_float(
            "MAX_SPEED_THRESHOLD_MPH", 2.0)
        self.ALERT_COOLDOWN_MINUTES = self._get_optional_int(
        ...
        self.USE_SQLITE_BACKEND = self._get_optional_bool(
            "USE_SQLITE_BACKEND", False)
        self.SQLITE_DB_PATH = self._get_optional(
            "SQLITE_DB_PATH", "bot_data.db")
        self.GSERVICE_ACCOUNT_JSON = self._get_optional(
            "GSERVICE_ACCOUNT_JSON", "service_account.json")
        ...
        self.MAX_CONCURRENT_JOBS = self._get_optional_int(
            "MAX_CONCURRENT_JOBS", 50)
        self.JOB_QUEUE_MAX_SIZE = self._get_optional_int(
            "JOB_QUEUE_MAX_SIZE", 1000)

        # API Rate Limiting
        self.TMS_REQUEST_DELAY = self._get_optional_float(
            "TMS_REQUEST_DELAY", 1.0)
        self.TMS_MAX_REQUESTS_PER_MINUTE = self._get_optional_int(
        ...
        self.MAX_RETRY_ATTEMPTS = self._get_optional_int(
            "MAX_RETRY_ATTEMPTS", 3)
        self.RETRY_DELAY_SECONDS = self._get_optional_int(
            "RETRY_DELAY_SECONDS", 5)
        self.RETRY_EXPONENTIAL_BACKOFF = self._get_optional_bool(
        ...
        # Core Features
        self.ENABLE_PM_TRACKING = self._get_optional_bool(
            "ENABLE_PM_TRACKING", True)
        self.ENABLE_GROUP_AUTO_UPDATES = self._get_optional_bool(
            "ENABLE_GROUP_AUTO_UPDATES", True)
        self.ENABLE_LIVE_TRACKING = self._get_optional_bool(
            "ENABLE_LIVE_TRACKING", True)

        # Advanced Features
        self.ENABLE_ROUTE_OPTIMIZATION = self._get_optional_bool(
            "ENABLE_ROUTE_OPTIMIZATION", True)
        self.ENABLE_TRAFFIC_DATA = self._get_optional_bool(
            "ENABLE_TRAFFIC_DATA", True)
        self.ENABLE_WEATHER_INTEGRATION = self._get_optional_bool(
        ...
        self.SEND_ERROR_NOTIFICATIONS = self._get_optional_bool(
            "SEND_ERROR_NOTIFICATIONS", True)
        self.SEND_STATUS_UPDATES = self._get_optional_bool(
            "SEND_STATUS_UPDATES", True)
        self.NOTIFICATION_COOLDOWN_MINUTES = self._get_optional_int(
            "NOTIFICATION_COOLDOWN_MINUTES", 15)

        # Message Formatting
        self.USE_MARKDOWN_MESSAGES = self._get_optional_bool(
            "USE_MARKDOWN_MESSAGES", True)
        self.ENABLE_MAP_PREVIEWS = self._get_optional_bool(
            "ENABLE_MAP_PREVIEWS", True)
        self.MAX_MESSAGE_LENGTH = self._get_optional_int(
        ...
        self.LOCATION_ADDR_CACHE_TTL_SECS = self._get_optional_int(
            "LOCATION_ADDR_CACHE_TTL_SECS", 86400)  # 24 hours
        self.RENDER_COORD_DECIMALS = self._get_optional_int(
            "RENDER_COORD_DECIMALS", 5)  # ~1.1m precision
        self.REVERSE_GEOCODE_TIMEOUT_SECS = self._get_optional_int(
            "REVERSE_GEOCODE_TIMEOUT_SECS", 5)

        # Feature flag for location rendering
        self.ENABLE_NEW_LOCATION_RENDERER = self._get_optional_bool(
            "ENABLE_NEW_LOCATION_RENDERER", True)
        ...
        self.GEOCODING_CACHE_SIZE = self._get_optional_int(
            "GEOCODING_CACHE_SIZE", 1000)
        self.ROUTE_CACHE_SIZE = self._get_optional_int("ROUTE_CACHE_SIZE", 500)
        self.TRUCK_DATA_CACHE_TTL = self._get_optional_int(
            "TRUCK_DATA_CACHE_TTL", 120)  # 2 minutes

        # Data Quality Settings
        self.MAX_LOCATION_AGE_HOURS = self._get_optional_int(
            "MAX_LOCATION_AGE_HOURS", 12)
        self.MIN_COORDINATE_PRECISION = self._get_optional_float(
            "MIN_COORDINATE_PRECISION", 0.001)
        self.MAX_ROUTE_DISTANCE_MILES = self._get_optional_int(
        ...
        self.ENABLE_DEBUG_LOGGING = self._get_optional_bool(
            "ENABLE_DEBUG_LOGGING", False)

        # Detailed Logging Options
        self.LOG_TMS_REQUESTS = self._get_optional_bool(
            "LOG_TMS_REQUESTS", False)
        self.LOG_ORS_REQUESTS = self._get_optional_bool(
            "LOG_ORS_REQUESTS", False)
        self.LOG_USER_INTERACTIONS = self._get_optional_bool(
            "LOG_USER_INTERACTIONS", True)
        self.LOG_JOB_EXECUTION = self._get_optional_bool(
            "LOG_JOB_EXECUTION", True)

        # Dashboard Logging Settings
        self.ENABLE_DASHBOARD_LOGGING = self._get_optional_bool(
            "ENABLE_DASHBOARD_LOGGING", True)
        self.LOG_FLUSH_INTERVAL = self._get_optional_int(
            "LOG_FLUSH_INTERVAL", 300)  # 5 minutes
        self.SHEETS_BATCH_SIZE = self._get_optional_int(
            "SHEETS_BATCH_SIZE", 50)

        # Worksheet Names for Logging
        self.SPREADSHEET_DASHBOARD = self._get_optional(
            "SPREADSHEET_DASHBOARD", "dashboard_logs")
        self.SPREADSHEET_FLEET_STATUS = self._get_optional(
            "SPREADSHEET_FLEET_STATUS", "fleet_status")
        ...
        self.MAX_CONCURRENT_TELEGRAM_SENDS = self._get_optional_int(
            "MAX_CONCURRENT_TELEGRAM_SENDS", 12)
        self.SCHEDULER_JITTER_MAX_SECONDS = self._get_optional_int(
            "SCHEDULER_JITTER_MAX_SECONDS", 15)
        self.SCHEDULER_RETRY_MAX_ATTEMPTS = self._get_optional_int(
            "SCHEDULER_RETRY_MAX_ATTEMPTS", 3)

        # Distribution Settings for Large Fleets
        self.ENABLE_SCHEDULER_JITTER = self._get_optional_bool(
            "ENABLE_SCHEDULER_JITTER", True)
        self.SCHEDULER_DISTRIBUTION_WINDOW = self._get_optional_int(
            "SCHEDULER_DISTRIBUTION_WINDOW", 900)  #
        # =====================================================
...
"""
    ...
    def _validate_critical_settings(self):
        ...
        if not Path(self.SHEETS_SERVICE_ACCOUNT_FILE).exists():
            warnings.append(
                f"Service account file not found: {self.SHEETS_SERVICE_ACCOUNT_FILE}")

        # Validate intervals
        if self.GROUP_LOCATION_INTERVAL < 300:  # 5 minutes minimum
            warnings.append(
                "GROUP_LOCATION_INTERVAL should be at least 300 seconds (5 minutes)")

        if self.LIVE_TRACKING_INTERVAL < 60:  # 1 minute minimum
            warnings.append(
                "LIVE_TRACKING_INTERVAL should be at least 60 seconds for API rate limits")
    ...
    def _log_configuration_summary(self):
        ...
        if self.ENABLE_DEBUG_LOGGING:
            ...
            logger.info(
                f"Group Location Messages: {self.GROUP_LOCATION_INTERVAL}s ({self.GROUP_LOCATION_INTERVAL//60} min)")
            logger.info(
                f"Silent Data Refresh: {self.LIVE_TRACKING_INTERVAL}s ({self.LIVE_TRACKING_INTERVAL//60} min)")
            logger.info(
                f"Auto-start location updates: {self.AUTO_START_LOCATION_UPDATES}")
            logger.info(f"Max live sessions: {self.MAX_LIVE_SESSIONS}")
            logger.info(f"Max group sessions: {self.MAX_GROUP_SESSIONS}")
            logger.info(
                f"Risk monitoring enabled: {self.ENABLE_RISK_MONITORING}")
            logger.info(
                f"QC team chat ID: {'Configured' if self.QC_TEAM_CHAT_ID else 'Not configured'}")
            logger.info(
                f"QC Panel spreadsheet: {'Configured' if self.QC_PANEL_SPREADSHEET_ID else 'Not configured'}")
            logger.info(
                f"Features enabled: PM={self.ENABLE_PM_TRACKING}, Groups={self.ENABLE_GROUP_AUTO_UPDATES}, Live={self.ENABLE_LIVE_TRACKING}")
            logger.info("==============================")
