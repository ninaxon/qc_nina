...
class RobustSheetsWriter:
    ...
    def __init__(self, worksheet, chunk_size: int = 300, allow_new_trucks: bool = False):
        self.worksheet = worksheet
        self.chunk_size = chunk_size
        self.allow_new_trucks = allow_new_trucks  # Safety flag to prevent automatic VIN addition
    ...
    def unmerge_all_cells(self):
        ...
        try:
            ...
            # For now, we'll skip this and assume it's done by fix_sheets_merges.py
            logger.debug("Merged cells should be cleared by fix_sheets_merges.py")
        except ImportError:
            logger.debug("Google API client not available for unmerge operation")
    
    def write_tms_data_to_assets(self, trucks: List[Dict], existing_records: List[Dict], 
        ...
        try:
            logger.info(f"Starting robust TMS → assets update for {len(trucks)} trucks")
            logger.info(f"New truck addition is {'ENABLED' if self.allow_new_trucks else 'DISABLED'} (safety mode)")
            ...
            for i, record in enumerate(existing_records):
                ...
                if vin:
                    vin_to_row[vin] = i + 2  # +2 for header row and 1-based indexing
            
            logger.info(f"Found {len(vin_to_row)} existing VINs in assets sheet")
            ...
            for truck in trucks:
                ...
                try:
                    ...
                    if vin in vin_to_row:
                        ...
                        # Find column indices
                        location_col = self._find_header_col(headers, 'Last Known Location')
                        ...
                        update_col = self._find_header_col(headers, 'Update Time')
                        ...
                        if self.allow_new_trucks:
                            ...
                            self._set_row_value(new_row, headers, 'Last Known Location', location)
                            self._set_row_value(new_row, headers, 'Latitude', lat_str)  
                            self._set_row_value(new_row, headers, 'Longitude', lon_str)
                            self._set_row_value(new_row, headers, 'Status', status)
                            self._set_row_value(new_row, headers, 'Update Time', current_time)
                            self._set_row_value(new_row, headers, 'Source', 'TMS')
                    ...
                    stats['errors'].append(f"Error processing truck {vin}: {e}")
                    logger.debug(f"Error processing truck {vin}: {e}")
                    continue
            
            # Execute batch updates in chunks
            if batch_updates:
                logger.info(f"Executing {len(batch_updates)} cell updates in chunks of {self.chunk_size}")
                
                for i, chunk in enumerate(chunked(batch_updates, self.chunk_size)):
                    try:
                        # Execute chunk with retry
                        self._execute_batch_update_chunk(chunk, i + 1, len(list(chunked(batch_updates, self.chunk_size))))
                        time.sleep(0.1)  # Small delay between chunks
                        
                    except Exception as e:
                        stats['errors'].append(f"Batch update chunk {i+1} failed: {e}")
                        logger.error(f"Batch update chunk {i+1} failed: {e}")
            
            # Add new rows if any (and if allowed)
            if new_rows and self.allow_new_trucks:
                logger.info(f"Adding {len(new_rows)} new trucks to assets sheet")
                try:
                    # Ensure sheet is large enough
                    current_rows = self.worksheet.row_count
                    needed_rows = len(existing_records) + len(new_rows) + 1  # +1 for header
                    ...
                    start_row = len(existing_records) + 2  # +2 for header and 1-based indexing
                    
                    for chunk in chunked(new_rows, self.chunk_size):
                        end_row = start_row + len(chunk) - 1
                        range_name = a1_range(start_row, 1, end_row, len(headers))
                        
                        self.worksheet.update(range_name, chunk, value_input_option="RAW")
                        logger.debug(f"Added chunk: rows {start_row}-{end_row}")
                ...
                logger.warning(f"Skipped adding {len(new_rows)} new trucks (auto-addition disabled for safety)")
            
            duration = time.time() - start_time
            logger.info(f"TMS → assets update completed in {duration:.1f}s")
            logger.info(f"Stats: {stats['trucks_processed']} processed, {stats['assets_updated']} updated, "
            ...
            logger.error(f"Critical error in robust write operation: {e}", exc_info=True)
            return stats
    
    def _execute_batch_update_chunk(self, chunk: List[Dict], chunk_num: int, total_chunks: int):
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.debug(f"Batch chunk {chunk_num}/{total_chunks} completed ({len(chunk)} updates)")
                return
                ...
                logger.warning(f"Batch chunk {chunk_num} attempt {attempt + 1} failed, retrying in {wait_time}s: {e}")
    ...
    def _set_row_value(self, row: List, headers: List[str], header: str, value: str):
