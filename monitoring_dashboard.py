...
class LiveUpdateMonitor:
    ...
    def get_system_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'system': {
                ...
                'load_avg': psutil.getloadavg() if hasattr(
                    psutil,
        ...
        if process:
            try:
                stats['bot_process'] = {
                    ...
                    'connections': len(
                        process.connections()) if hasattr(
                        process,
                        'connections') else 0}
    ...
    def _get_recent_errors(self, since: datetime,
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        errors_1h = report['log_analysis_1h'].get(
            'live_updates', {}).get('errors', 0)
        success_1h = report['log_analysis_1h'].get(
            'live_updates', {}).get('success', 0)
        if success_1h > 0:
        ...
        live_updates = report['log_analysis_1h'].get(
            'live_updates', {}).get('count', 0)
        if live_updates > 0:
    ...
    def display_live_dashboard(self):
        ...
        def generate_dashboard():
            ...
            # Header
            header_text = Text(
                "🔴 Live Asset Tracking Bot Dashboard",
                style="bold magenta")
            header_text.append(
            ...
            system_table.add_row(
                "Memory", f"{sys_stats['memory_percent']:.1f}%")
            ...
            if report['system']['bot_running']:
                ...
                process_table.add_row(
                    "Memory", f"{proc_stats['memory_mb']} MB")
                process_table.add_row(
                    "CPU", f"{proc_stats['cpu_percent']:.1f}%")
            ...
            if errors:
                error_text = "\n".join(
                    [f"• {err['message']}" for err in errors[:5]])
            else:
    ...
    def display_simple_status(self):
        ...
        print(
            f"   Live Updates: {live_updates.get('count', 0)} (Errors: {live_updates.get('errors', 0)})")
        print(
            f"   TMS API Calls: {tms_calls.get('count', 0)} (Errors: {tms_calls.get('errors', 0)})")
        print(
            f"   Jobs Scheduled: {jobs.get('scheduled', 0)} (Failed: {jobs.get('failed', 0)})")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Live Update Bot Monitoring Dashboard")
    parser.add_argument(
        "--live",
        action="store_true",
